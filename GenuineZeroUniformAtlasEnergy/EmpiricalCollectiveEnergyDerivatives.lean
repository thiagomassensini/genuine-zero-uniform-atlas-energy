...
lemma hasDerivAt_criticalLineParameter (time : ℝ) :
  ...
  have hfun : criticalLineParameter =
...
lemma hasDerivAt_criticalLine_firstJet_comp
    (F : ℂ → ℂ) (time : ℝ)
    (_hF : HasDerivAt F (iteratedDeriv 1 F (criticalLineParameter time))
  ...
  have hderiv :
    ...
    calc
      criticalLineSecondJet F time =
          -iteratedDeriv 2 F (criticalLineParameter time) := rfl
      _ = (0 : ℂ) * iteratedDeriv 1 F (criticalLineParameter time) +
          Complex.I * (Complex.I *
            iteratedDeriv 2 F (criticalLineParameter time)) := by
        simp only [zero_mul, zero_add, ← mul_assoc, Complex.I_mul_I,
          neg_one_mul]
  rw [hfun, hderiv]
  exact hmul
...
lemma analyticAt_empiricalNativeTailCoefficient_critical
    ...
    AnalyticAt ℂ (empiricalNativeTailCoefficient camera)
      (criticalLineParameter time) := by
  cases camera <;>
    unfold empiricalNativeTailCoefficient nativeExplicitRadiusTailCoefficient
...
lemma abs_complexEnergyPair_le (z w : ℂ) :
  ...
  calc
    |complexEnergyPair z w| = 2 * |(inner ℂ z w).re| := by
      ...
      norm_num
    _ ≤ 2 * ‖inner ℂ z w‖ :=
      mul_le_mul_of_nonneg_left (Complex.abs_re_le_norm _) htwo
    _ ≤ 2 * ‖z‖ * ‖w‖ := by
      convert mul_le_mul_of_nonneg_left (norm_inner_le_norm z w) htwo using 1 <;>
        ring
...
lemma abs_complexEnergyPair_sub_le_of_norm_sub_le
  ...
  calc
    ...
    _ ≤ 2 * ‖z - a‖ * ‖b‖ +
      ...
      exact add_le_add
        (add_le_add
          (abs_complexEnergyPair_le (z - a) b)
          (abs_complexEnergyPair_le a (w - b)))
        (abs_complexEnergyPair_le (z - a) (w - b))
    _ ≤ 2 * (‖a‖ * e₁ + ‖b‖ * e₀ + e₀ * e₁) := by
...
lemma empiricalLeadingCollectiveCoefficientEnergyFirst_eq (time : ℝ) :
  ...
  have hquad : HasDerivAt
    ...
    simpa [Pi.add_apply, Pi.mul_apply, id_eq, pow_two] using hscaled
...
lemma empiricalScaledCameraTailFirstJetConstant_nonneg
  ...
  exact div_nonneg
    ...
    (by norm_num [nativeExplicitRadiusCriticalCauchyRadius])
...
lemma norm_empiricalScaledCameraTailErrorCritical_value_le_constant_div
  ...
  simpa [empiricalScaledCameraTailErrorCritical,
    empiricalScaledCameraTailValueJetConstant] using
...
lemma norm_empiricalScaledCameraTailErrorCritical_first_le_constant_div
  ...
  unfold empiricalScaledCameraTailFirstJetConstant
  convert h using 1
  · field_simp [nativeExplicitRadiusCriticalCauchyRadius,
      show (M : ℝ) ≠ 0 by
...
lemma empiricalScaledCameraTailEnergyFirst_remainder_le
  ...
  have h0 : ‖empiricalScaledCameraCutoffTail camera M time -
      empiricalNativeTailCoefficientCritical camera time‖ ≤
      empiricalScaledCameraTailValueJetConstant camera time / (M : ℝ) := by
    have hdiff : empiricalScaledCameraCutoffTail camera M time -
        empiricalNativeTailCoefficientCritical camera time =
        empiricalScaledCameraTailErrorCritical camera M time := by
